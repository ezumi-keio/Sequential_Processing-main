...
def _normalize(img_lst, mean, std):
    """Normalize tensor input images
    ...
    def _main(img):
        ...
        img_n = img_transform(img)
        return img_n

    if isinstance(img_lst, list):
        return_lst = []
        return_lst.append(_main(img_lst[0]))
        return_lst.append(img_lst[1])
        return return_lst
...
class DiskIODataset(Dataset):
    ...
    def __init__(self, gt_path, lq_path, mean, std, dif_lqgt, imgformat_gt, imgformat_lq, if_train, if_baseline, max_num=-1, start_idx=0, aug=None, center_crop=None, padding=None, split=None):
        super().__init__()
        ...
        self.opts_norm_mean = mean if (mean is not None) else None
        self.opts_norm_std = std if (std is not None) else None
        self.dif_lqgt = str(dif_lqgt)
        self.imgformat_gt = str(imgformat_gt)
        self.imgformat_lq = str(imgformat_lq)
        # dataset path
        ...
        lq_lst = sorted(list(self.lq_path.glob('*' + self.imgformat_lq)))
        if max_num != -1:
        ...
        for idx, lq_path in enumerate(lq_lst):
            ...
            name_gt = name.split(self.dif_lqgt)[0]
            imgformat = lq_path.suffix
            gt_path = self.gt_path / (name_gt + imgformat) if self.gt_path is not None else None
    ...
    def __getitem__(self, idx):
        ...
        #img_L = cv2.cvtColor(img_lq, cv2.COLOR_RGB2BGR)                
        #_, encimg = cv2.imencode('.webp', img_L, [int(cv2.IMWRITE_WEBP_QUALITY), 10])
        #img_L = cv2.imdecode(encimg, 3)
        #img_lq = cv2.cvtColor(img_L, cv2.COLOR_BGR2RGB)               
        ...
        if not (self.if_baseline or self.opts_norm_mean == None or self.opts_norm_std == None):
            img_lst[0] = _normalize(img_lst[0], self.opts_norm_mean, self.opts_norm_mean)
