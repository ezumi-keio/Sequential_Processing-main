...
class BaseAlg:
    ...
    def accum_gradient(self, module, stage, group, data, inter_step, **_):
        ...
        data_out, qe_est = module(inp_t=data_lq, if_train=True)
        ...
        for loss_name in self.loss_lst[stage][group]:
            ...
            loss_unweighted = loss_dict['fn'](inp=data_out, ref=data_gt, q_inp=qe_est, q_ref=.5)
    ...
    def test(self, data_fetcher, num_samples, if_baseline=False, if_return_each=False, img_save_folder=None,
        ...
        while test_data is not None:
            ...
            timer.start_record()
            if if_tar_only:
                im_out, qe_est = self.model.net[self.model.infer_subnet](inp_t=im_lq, if_train=False).clamp_(0., 1.)
                timer.record_inter()
            else:
