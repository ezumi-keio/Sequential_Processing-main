...
loss_lst = ['CharbonnierLoss', 'CrossEntropyLoss', 'GANLoss', 'RelativisticGANLoss', 'VGGLoss',
             'MSELoss','MSRCLoss', 'MSFDLoss', 'CompIELoss', 'MapQELoss']
...
class MSRCLoss(nn.Module):
    def __init__(self, reduction='mean'):
        super().__init__()
        self.loss = nn.L1Loss(reduction=reduction)

    def forward(self, inp, ref, q_inp, q_ref):
        return self.loss(inp, ref)


class CompIELoss(nn.Module):
    def __init__(self):
        super().__init__()

    def forward(self, inp, ref, q_inp, q_ref):
        return abs(q_inp - q_ref)

class MapQELoss(nn.Module):
    def __init__(self, reduction='mean'):
        super().__init__()
        self.loss = nn.L1Loss(reduction=reduction)
    ...
    def forward(self, inp, ref, q_inp, q_ref):
        ...
        return self.loss(q_inp, q_ref)


class MSFDLoss(nn.Module):
    def __init__(self, reduction='mean'):
        super().__init__()
        self.loss = nn.L1Loss(reduction=reduction)

    def forward(self, inp, ref, q_inp, q_ref):
        inp_fft = torch.fft.rfft2(inp)
        ref_fft = torch.fft.rfft2(ref)
        return self.loss(inp_fft, ref_fft)
